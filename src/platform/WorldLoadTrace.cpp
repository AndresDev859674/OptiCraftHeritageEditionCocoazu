...
// The thread that called beginLoad(). The stack above is static and unlocked,
// and the async generation worker runs marked-up code (generateBaseTerrain,
// the cave and ravine passes) while the game thread is still inside the load,
// so every entry point below is inert on any other thread. That worker's
// stages also belong to no step of this tree.
std::uintptr_t s_loadThread = 0;

bool onLoadThread()
{
    return s_active && PlatformThread::currentId() == s_loadThread;
}
...
void beginLoad(const char* what)
{
    ...
    s_overflow = 0;
    s_active = true;
    s_loadThread = PlatformThread::currentId();
