...
void renderTextureParameters(bool blur, bool mipmaps, bool clamp)
{
    ...
    GXTexObj object;
    if (wii_native_texture_get_current(&texture, &object))
        wii_native_texture_set_parameters(texture, blur, mipmaps, clamp);
}
...
bool renderTextureBeginUpload(int texture, int width, int height, int maxLevel, bool blur, bool clamp, bool, bool highPrecision)
{
    renderBindTexture(texture);
    return wii_native_texture_begin_upload(texture, width, height, maxLevel, blur, clamp, highPrecision);
}
...
void renderFogf(RenderFogParameter parameter, float value)
{
    ...
    switch (parameter)
    {
        ...
        case RenderFogParameter::DistanceMode: nativeParameter = WiiRenderFogParameter::DistanceMode; break;
    }
...
void renderFogi(RenderFogParameter parameter, RenderFogMode value)
{
    ...
    switch (value)
    {
        ...
        case RenderFogMode::EyeRadial: nativeMode = WiiRenderFogMode::EyeRadial; break;
    }
    const WiiRenderFogParameter nativeParameter = parameter == RenderFogParameter::DistanceMode
        ? WiiRenderFogParameter::DistanceMode
        : WiiRenderFogParameter::Mode;
    wii_gx_native_fogi(wiiRenderValue(nativeParameter), static_cast<int>(wiiRenderValue(nativeMode)));
}
...
bool renderSupportsFeature(RenderFeature feature)
{
    switch (feature)
    {
        ...
        case RenderFeature::FancyFogDistance: return true;
...
{
    wiigl_set_legacy_gamma_enabled(enabled);
}
...
bool renderCompileTerrainMesh(int handle, const RenderInterleavedMesh& mesh, const RenderTerrainCompileInfo& info)
{
    ...
    // Vanilla 1.2.5 feeds the packed brightness field to texture unit 1 while
    // rendering chunk display lists. Preserve it in the GX display list so the
    // dynamic lightmap, including the brightness slider, matches that path.
    ...
    return wii_native_compile_chunk_list(handle, mesh.data, mesh.stride,
        mesh.hasTexture, mesh.hasColor, mesh.hasBrightness, mesh.brightnessOffset,
        static_cast<WiiRenderPrimitive>(renderPrimitiveValue(mesh.primitive)), mesh.first, mesh.count,
