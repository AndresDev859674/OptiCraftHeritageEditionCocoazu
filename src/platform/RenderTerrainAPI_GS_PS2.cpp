...
std::size_t renderTerrainStagingBytes() { return ps2_mesh_staging_ram_bytes() + ps2_terrain_command_ram_bytes(); }
...
static RenderTerrainCacheBuildStatus cacheBuildStatus(Ps2TerrainMeshBuildStepResult result)
{
    switch (result)
    {
    case Ps2TerrainMeshBuildStepResult::Pending:
        return RenderTerrainCacheBuildStatus::Pending;
    case Ps2TerrainMeshBuildStepResult::Complete:
        return RenderTerrainCacheBuildStatus::Complete;
    default:
        return RenderTerrainCacheBuildStatus::Failed;
    }
}

RenderTerrainCacheBuildStatus renderTerrainCacheBeginOpaqueBuild(RenderTerrainBackendCache& cache,
                                   const int_t* raw, std::size_t rawIntCount, int_t vertexCount,
                                   int_t drawMode, bool hasTexture, bool hasColor, bool hasNormals,
                                   bool& didWork)
{
    if (!cache.initialized) renderTerrainCacheInit(cache);
    return cacheBuildStatus(cacheMesh(cache)->beginBuild(raw, rawIntCount, vertexCount,
        drawMode, hasTexture, hasColor, hasNormals, didWork));
}

RenderTerrainCacheBuildStatus renderTerrainCacheContinueOpaqueBuild(RenderTerrainBackendCache& cache,
                                      bool& didWork)
{
    ...
    return cacheBuildStatus(cacheMesh(cache)->continueBuild(didWork));
}

void renderTerrainCacheCancelOpaqueBuild(RenderTerrainBackendCache& cache)
{
    if (cache.initialized)
        cacheMesh(cache)->cancelBuild();
}

void renderTerrainCacheSwap(RenderTerrainBackendCache& a, RenderTerrainBackendCache& b)
{
    if (!a.initialized) renderTerrainCacheInit(a);
    if (!b.initialized) renderTerrainCacheInit(b);
    using std::swap;
    swap(*cacheGroups(a), *cacheGroups(b));
    cacheMesh(a)->swap(*cacheMesh(b));
}
