...
constexpr float kPolygonOffsetFactorWeight = 0.25f;
}
...
{
    std::memcpy(projection, source, sizeof(Mtx44));
    ...
    // GX has no slope-scaled polygon depth bias. Minecraft 1.2.5 uses
    // factor and units together only for its coplanar block-damage overlay,
    // so preserve the units bias and fold in a conservative fraction of the
    // slope term instead of silently discarding factor.
    const float polygonOffset = polygonOffsetUnits +
        polygonOffsetFactor * kPolygonOffsetFactorWeight;
    const float zOffset = polygonOffsetFill
        ? polygonOffset * kPolygonOffsetScale
        : 0.0f;
                                 ...
                                 float polygonOffsetFactor, float polygonOffsetUnits)
{
    Mtx44 projection;
    u8 type;
    wiiBuildProjectionFromOpenGL(source, polygonOffsetFill, polygonOffsetFactor,
                                 polygonOffsetUnits, projection, type);
