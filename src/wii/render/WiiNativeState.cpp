...
static u8 alphaReferenceToGx(unsigned int compare, float value)
{
    const float scaled = clamp01(value) * 255.0f;
    float quantized = 0.0f;
    switch (compare)
    {
        case wiiRenderValue(WiiRenderCompare::Greater):
        case wiiRenderValue(WiiRenderCompare::LessEqual):
            quantized = std::floor(scaled);
            break;
        case wiiRenderValue(WiiRenderCompare::Less):
        case wiiRenderValue(WiiRenderCompare::GreaterEqual):
            quantized = std::ceil(scaled);
            break;
        default:
            quantized = std::floor(scaled + 0.5f);
            break;
    }
    return static_cast<u8>(std::max(0.0f, std::min(255.0f, quantized)));
}
...
static u8 blendToGx(unsigned int value)
{
    switch (value)
    {
        ...
        case wiiRenderValue(WiiRenderBlendFactor::DstAlpha): return GX_BL_ONE;
        case wiiRenderValue(WiiRenderBlendFactor::OneMinusDstAlpha): return GX_BL_ZERO;
...
static void initializeDefaults()
{
    ...
    s_state.snapshot.fog_eye_radial = 0;
...
extern "C" void wii_gx_native_alpha_func(unsigned int func, float ref)
{
    ...
    s_state.snapshot.alpha_ref = alphaReferenceToGx(func, ref);
}
...
extern "C" void wii_gx_native_fogf(unsigned int pname, float value)
{
    ...
    switch (pname)
    {
        ...
        case wiiRenderValue(WiiRenderFogParameter::DistanceMode):
            s_state.snapshot.fog_eye_radial =
                static_cast<int>(value) == wiiRenderValue(WiiRenderFogMode::EyeRadial) ? 1 : 0;
            return;
        default: return;
    }
    syncFog();
...
extern "C" void wii_gx_native_polygon_offset(float factor, float units)
{
    initializeDefaults();
    s_state.snapshot.polygon_offset_factor = factor;
...
extern "C" const unsigned char* wii_gx_native_get_string(unsigned int name)
{
    switch (name)
    {
        ...
        case wiiRenderValue(WiiRenderStringQuery::Renderer): return reinterpret_cast<const unsigned char*>("OptiCraft native GX");
...
extern "C" void wii_gx_native_forget_texture_id(int texture)
{
    initializeDefaults();
    bool changed = false;
    if (s_state.snapshot.texture0_id == texture)
    {
        s_state.snapshot.texture0_id = 0;
        changed = true;
    }
    if (s_state.snapshot.texture1_id == texture)
    {
        s_state.snapshot.texture1_id = 0;
        changed = true;
    }
...
extern "C" void wii_gx_native_clear_bound_texture_ids(void)
{
    initializeDefaults();
    if (s_state.snapshot.texture0_id == 0 && s_state.snapshot.texture1_id == 0)
        return;
