...
namespace
{
	...
	constexpr std::uint8_t kWiiAllCubeFaces = 0x3fu;

	bool wiiOpaqueNeighbour(ChunkCache &cache, int_t x, int_t y, int_t z)
	{
	...
	std::uint8_t wiiExposedCubeFaceMask(ChunkCache &cache, int_t x, int_t y, int_t z)
	{
		std::uint8_t opaqueMask = 0;
		if (wiiOpaqueNeighbour(cache, x, y - 1, z)) opaqueMask |= 1u << WiiFaceDown;
		if (wiiOpaqueNeighbour(cache, x, y + 1, z)) opaqueMask |= 1u << WiiFaceUp;
		if (wiiOpaqueNeighbour(cache, x, y, z - 1)) opaqueMask |= 1u << WiiFaceNorth;
		if (wiiOpaqueNeighbour(cache, x, y, z + 1)) opaqueMask |= 1u << WiiFaceSouth;
		if (wiiOpaqueNeighbour(cache, x - 1, y, z)) opaqueMask |= 1u << WiiFaceWest;
		if (wiiOpaqueNeighbour(cache, x + 1, y, z)) opaqueMask |= 1u << WiiFaceEast;
		return static_cast<std::uint8_t>((~opaqueMask) & kWiiAllCubeFaces);
	}
}
...
bool WorldRenderer::wiiBuildRendererStep(int_t blockBudget)
{
	...
	while (wiiBuildPass < 2)
	{
		...
		while (wiiBuildCursor < totalBlocks && processed < blockBudget)
		{
			...
			if (id > 0)
			{
				...
				const WiiBlockRenderInfo &renderInfo = wiiGetBlockRenderInfo(id);
				...
				std::uint8_t exposedFaceMask = kWiiAllCubeFaces;
		...
		if (tessellator->capture(stepMesh))
		{
			...
			wiiBuildHasBrightness[wiiBuildPass] |= stepMesh.hasBrightness;
		...
		if (wiiBuildVertexCount[wiiBuildPass] > 0 && !wiiBuildRawBuffer[wiiBuildPass].empty())
		{
			...
			mesh.colorOffset = 20;
			mesh.hasBrightness = wiiBuildHasBrightness[wiiBuildPass];
	...
	const bool dirtyDuringBuild = wiiBuildDirtyDuringBuild;
