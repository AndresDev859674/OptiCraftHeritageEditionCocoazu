...
const char* const kAppSubDir = "/apps/OptiCraft";
const char* const kProbeFile = "/data/assets/font.txt";
// An install packed with scripts/make_pak.py ships this beside data/ and may
// carry no loose data/assets at all; AssetPak mounts it from the app dir.
const char* const kPakFile = "/assets.pak";
...
bool hasGameData(const char* dir)
{
	...
	if (fileExists(path))
		return true;
	std::snprintf(path, sizeof(path), "%s%s", dir, kPakFile);
...
void reportStorageLayout()
{
	...
	if (!g_appDirHasData)
	{
		...
		MC_LOG_INFO("wii", "     Copy bin\\wii\\sd\\apps\\OptiCraft to an SD card\n");
...
bool wiiEnsureStorage()
{
	...
	else
	{
		...
		MC_LOG_INFO("wii", "  apps/OptiCraft/ , or in Dolphin:\n");
