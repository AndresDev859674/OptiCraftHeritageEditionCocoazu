...
constexpr int kMenuPointerActivationPixels = 6;
...
bool g_menuPointerActivity = false;
bool g_menuActivityOriginValid = false;
int g_menuActivityOriginX = 0;
int g_menuActivityOriginY = 0;
int g_irPreviousX = 0;
int g_irPreviousY = 0;
int g_cursorX = 0;
int g_cursorY = 0;
...
int handleIr(int x, int y, bool allowGameplayLook)
{
	...
	if (!g_menuActivityOriginValid)
	{
		g_menuActivityOriginX = x;
		g_menuActivityOriginY = y;
		g_menuActivityOriginValid = true;
	}
	...
	if (g_haveIrPrevious)
	{
		...
		if (dx != 0 || dy != 0)
		{
			g_cursorX = x;
			g_cursorY = y;
			const int activityDx = x - g_menuActivityOriginX;
			const int activityDy = y - g_menuActivityOriginY;
			if (activityDx <= -kMenuPointerActivationPixels || activityDx >= kMenuPointerActivationPixels ||
				activityDy <= -kMenuPointerActivationPixels || activityDy >= kMenuPointerActivationPixels)
			{
				g_menuPointerActivity = true;
				g_menuActivityOriginX = x;
				g_menuActivityOriginY = y;
			}
...
bool menuPointerActivity()
{
	return g_menuPointerActivity;
}

void resetMenuActivityBaseline()
{
	g_menuActivityOriginValid = false;
}
...
void centerCursor()
{
	setCursorPosition(wiigl_width() / 2, wiigl_height() / 2);
}
