...
void reportStorage()
{
	...
	for (const char *device : devices)
	{
		...
		std::snprintf(probe, sizeof(probe),
		              "%s:/apps/OptiCraft/data/assets/font.txt", device);
		...
		if (f && !foundAppDir)
		{
			std::snprintf(appDir, sizeof(appDir), "%s:/apps/OptiCraft", device);
			foundAppDir = appDir;
		}
	}
	...
	if (!foundAppDir)
	{
		WiiConsole::write("STORAGE *** no apps/OptiCraft/data on any device ***\n");
...
int main(int argc, char **argv)
{
	...
	WiiConsole::write("\n\x1b[2;0H");
	WiiConsole::write("OptiCraft - Wii bring-up\n");
	WiiConsole::write("---------------------------\n\n");
