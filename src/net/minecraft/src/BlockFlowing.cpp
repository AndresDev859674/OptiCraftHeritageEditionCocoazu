...
int_t BlockFlowing::calculateFlowCost(World *world, int_t i, int_t j, int_t k, int_t l, int_t i1)
{
    ...
    for (int_t k1 = 0; k1 < 4; k1++)
    {
        ...
        if (k1 == 2) j2--;
        if (k1 == 3) j2++;
        if (neighborRejectsFlow(world, l1, i2, j2)) continue;
        if (!blockBlocksFlow(world, l1, i2 - 1, j2)) return l;
        // Vanilla looks four blocks out for a drop to steer toward; the search
        // fans out threefold per step, so the depth sets the cost of every
        // spreading water tick (PLATFORM_FLUID_FLOW_SEARCH_DEPTH).
        if (l >= PLATFORM_FLUID_FLOW_SEARCH_DEPTH) continue;
...
bool *BlockFlowing::getOptimalFlowDirections(World *world, int_t i, int_t j, int_t k)
{
    for (int_t l = 0; l < 4; l++)
    {
        ...
        if (l == 2) j2--;
        if (l == 3) j2++;
        if (neighborRejectsFlow(world, j1, i2, j2)) continue;
...
{
    return blockIdBlocksFlow(world->getBlockId(i, j, k));
}

bool BlockFlowing::blockIdBlocksFlow(int_t l) const
{
...
bool BlockFlowing::neighborRejectsFlow(World *world, int_t i, int_t j, int_t k)
{
    const int_t l = world->getBlockId(i, j, k);
    if (blockIdBlocksFlow(l))
        return true;
    // Air reads as Material::air through getBlockMaterial, never this liquid.
    return l != 0 && Block::blocksList[l]->blockMaterial == blockMaterial &&
