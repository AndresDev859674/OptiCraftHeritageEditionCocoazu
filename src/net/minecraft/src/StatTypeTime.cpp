...
#ifdef PS2_PLATFORM
	if (d4 > 0.5)
		return CompactNumberFormat::fixed2(d4) + " y";
	if (d3 > 0.5)
		return CompactNumberFormat::fixed2(d3) + " d";
	if (d2 > 0.5)
		return CompactNumberFormat::fixed2(d2) + " h";
	if (d1 > 0.5)
		return CompactNumberFormat::fixed2(d1) + " m";
#else
