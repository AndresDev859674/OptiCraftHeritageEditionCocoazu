...
{
	return getByteArrayRef(s);
}

std::vector<byte_t> NBTTagCompound::takeByteArray(const jstring &s)
{
	auto it = tagMap.find(s);
	if (it == tagMap.end()) return std::vector<byte_t>();
	NBTTagByteArray *tag = dynamic_cast<NBTTagByteArray *>(it->second);
	if (tag == nullptr) throw std::bad_cast();
	return std::move(tag->byteArray);
}

const std::vector<byte_t> &NBTTagCompound::getByteArrayRef(const jstring &s) const
{
	static const std::vector<byte_t> empty;
	auto it = tagMap.find(s);
	if (it == tagMap.end()) return empty;
	const NBTTagByteArray *tag = dynamic_cast<const NBTTagByteArray *>(it->second);
	if (tag == nullptr) throw std::bad_cast();
