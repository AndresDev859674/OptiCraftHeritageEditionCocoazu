...
EntityClientPlayerMP::EntityClientPlayerMP(Minecraft *minecraft, World *world, Session *session, NetClientHandler *netclienthandler)
	...
	, wasSneaking(false)
	, field_12242_bI(0)
	, motionUpdatesSentSinceWorldTick(false)
{
}
...
{
	motionUpdatesSentSinceWorldTick = true;
	if (field_9380_bx++ == 20)
	{
...
void EntityClientPlayerMP::flushMotionUpdatesForWorldTick()
{
	if (!motionUpdatesSentSinceWorldTick)
		sendMotionUpdates();
