...
public:
    ...
    explicit RegionFile(const std::string &path, bool readOnly = false);
    ~RegionFile();
...
#if PLATFORM_REGION_WHOLE_FILE_BUFFER
#  if PLATFORM_REGION_RANDOM_ACCESS
    // Read-only region access keeps only the 8 KiB header resident and fetches
    // requested sector ranges on demand. The first write materializes the full
    // region so the existing copy-on-write allocator and flush path stay intact.
    RegionStorageStream dataFile;
#  else
    ...
    RegionMemoryStream dataFile;
#  endif
#else
    // A real stream on disk, or a read-only view of a region inside assets.pak.
    RegionFileStream dataFile;
#endif
    int_t offsets[1024];
