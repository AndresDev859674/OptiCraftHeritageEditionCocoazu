...
void MetadataChunkBlock::updateLight(World *world)
{
...
#if PLATFORM_FAST_LIGHTING_CHUNK_ACCESS
    ...
    ExtendedBlockStorage *const *cachedSections = nullptr;
    ...
    auto resolveChunk = [&](int_t lx, int_t lz) -> Chunk *
    {
        ...
        if (!cacheValid || cx != cachedChunkX || cz != cachedChunkZ)
        {
            ...
            cachedChunk = world->blockExists(lx, 64, lz)
                ...
                : nullptr;
            cachedSections = cachedChunk != nullptr ? cachedChunk->getBlockStorageArray() : nullptr;
        }
        return cachedChunk;
    };

    // Coordinates are validated here, so the reads go straight to the section
    // nibble/byte arrays instead of through Chunk's checked accessors.
    const bool isSkyLight = skyBlock == EnumSkyBlock::Sky;
    auto fastSavedLight = [&](int_t lx, int_t ly, int_t lz) -> int_t
    {
        if (ly < 0) ly = 0;
        else if (ly >= WorldHeight::HEIGHT) ly = WorldHeight::MAX_Y;

        if (resolveChunk(lx, lz) == nullptr)
            return skyBlock->defaultLightValue;
        ExtendedBlockStorage *section = cachedSections[ly >> 4];
        if (section == nullptr)
            return skyBlock->defaultLightValue;
        return isSkyLight
            ? section->getExtSkylightValue(lx & 0xf, ly & 0xf, lz & 0xf)
            : section->getExtBlocklightValue(lx & 0xf, ly & 0xf, lz & 0xf);
    };
    ...
    auto fastBlockId = [&](int_t lx, int_t ly, int_t lz) -> int_t
    {
        ...
        if (resolveChunk(lx, lz) == nullptr)
            return 0;
        ExtendedBlockStorage *section = cachedSections[ly >> 4];
        return section != nullptr ? section->getExtBlockID(lx & 0xf, ly & 0xf, lz & 0xf) : 0;
    };
