...
public:
	...
	// Per-axis point test against block collision boxes instead of the full
	// AABB gather and sweep; see EntityFX.cpp.
	void moveEntity(double d, double d1, double d2) override;
#endif
	// Sampled every PLATFORM_PARTICLE_BRIGHTNESS_INTERVAL ticks of particle
	// age instead of every rendered frame.
...
private:
	int_t cachedBrightness;
	int_t cachedBrightnessAge;
};
