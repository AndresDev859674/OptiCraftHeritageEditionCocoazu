...
static std::string regionPath(const std::string &worldDir, int_t chunkX, int_t chunkZ, RegionFileCache::Format format, bool readOnly)
{
...
#ifndef PS2_PLATFORM
    if (!readOnly && s_createdRegionDirs.insert(regionDir).second)
...
std::shared_ptr<RegionFile> RegionFileCache::acquireRegionFile(const std::string &worldDir,
                                                               ...
                                                               Format format, bool readOnly)
{
    std::lock_guard<std::mutex> guard(cacheMutex);
    const std::string path = regionPath(worldDir, chunkX, chunkZ, format, readOnly);
    ...
    std::shared_ptr<RegionFile> rf = std::make_shared<RegionFile>(path, readOnly);
    cache[path] = rf;
#if PLATFORM_BOUNDED_WORLD
    touchRegionLru(path);
#endif
