...
void EntityAnimal::attackEntity(Entity *entity, float distance)
{
	...
	EntityAnimal *animal = entity->isAnimal() ? static_cast<EntityAnimal *>(entity) : nullptr;
	if (animal == nullptr)
		return;
...
Entity *EntityAnimal::findPlayerToAttack()
{
	...
	if (inLove > 0)
	{
		...
		{
			// The typed query already filtered on this class; isAnimal() only
			// guards the static_cast.
			EntityAnimal *animal = entity != nullptr && entity->isAnimal() ? static_cast<EntityAnimal *>(entity) : nullptr;
	...
	{
		// Every adult animal without a target runs this each tick. The chunk
		// query walks nine chunks of entity sections to find players; the world
		// keeps them in playerEntities already, and the box test is the same
		// one Chunk::getEntitiesOfTypeWithinAAAB applies.
		for (EntityPlayer *player : worldObj->playerEntities)
		{
			if (player == nullptr || player->boundingBox == nullptr || !player->boundingBox->intersectsWith(searchBox))
				continue;
			ItemStack *held = player->getCurrentEquippedItem();
			if (held != nullptr && isWheat(held))
				return player;
