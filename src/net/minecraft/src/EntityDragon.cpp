...
void EntityDragon::onLivingUpdate()
{
    ...
    // Reported as its own tick phase so the End fight shows up in the frame
    // log separately from the entTick total it is part of.
    const long_t dragonStartNs = System::nanoTime();
#endif
    updateDragon();
#if PLATFORM_PROFILE_RENDER_PHASES
    platformProfileTickPhase("dragon", System::nanoTime() - dragonStartNs);
#endif
}

void EntityDragon::updateDragon()
{
    ...
    if (!worldObj->multiplayerWorld && maxHurtTime == 0 &&
        (PLATFORM_DRAGON_COLLISION_TICK_DIVISOR <= 1 || ticksExisted % PLATFORM_DRAGON_COLLISION_TICK_DIVISOR == 0))
    {
...
void EntityDragon::onDeathUpdate()
{
    ...
    if (field_40178_aA == 200)
    {
        if (!worldObj->multiplayerWorld)
        {
            ...
            // Persisted so a regenerated central End chunk does not decorate a
            // new dragon after this one was beaten. The world save writes it.
            EndDragonState::get(worldObj)->setDefeated();
        }
        onEntityDeath();
        setEntityDead();
    }
