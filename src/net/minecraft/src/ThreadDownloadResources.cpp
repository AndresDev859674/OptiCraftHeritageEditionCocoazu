...
void ThreadDownloadResources::run()
{
	...
	try
	{
		...
		const std::string legacyFolder = PlatformStorage::join(resourcesFolder, "legacy");
		if (PlatformStorage::pathIsDirectory(legacyFolder))
			loadResource(legacyFolder, "legacy/");
	}
...
// Same walk as the directory version below, over the mounted assets.pak: the
// audio tree is registered from the pak's "resources/" entries and each file
// is installed under its pak:// path, which SoundManager reads back through
// PlatformResources / the stream openers. Returns false when the pak holds no
// resources at all so the loose tree is scanned instead.
bool ThreadDownloadResources::loadPakResources(const std::string &directoryKey, const std::string &s)
{
	...
	for (const std::string &name : names)
	{
		...
		const std::string key = directoryKey + "/" + name;
		if (AssetPak::isDirectory(key))
		{
			loadPakResources(key, s + name + "/");
			continue;
		}
