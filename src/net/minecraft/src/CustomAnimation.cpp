...
const byte_t *CustomAnimation::getFirstFrameData() const
{
	if (!isValid())
		return nullptr;
	const std::size_t frameBytes = getFrameByteCount();
	return frameBytes <= imageBytes.size() ? imageBytes.data() : nullptr;
}
