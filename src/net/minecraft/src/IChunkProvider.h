...
public:
	...
	virtual Chunk *getChunkIfExists(int_t i, int_t j)
	{
		return chunkExists(i, j) ? provideChunk(i, j) : nullptr;
	}
	...
	// Changes whenever chunkExists() can return a different result for the same coordinates.
	virtual std::uint32_t getChunkTopologyVersion() const { return 0; }
	virtual bool isChunkPopulationPending(int_t i, int_t j) const
	{
		(void)i;
		(void)j;
		return false;
	}
	virtual std::vector<Chunk *> getLoadedChunksSnapshot() const { return {}; }
