...
namespace GameResources
{
    ...
    // Returns the path if it exists, or an empty string otherwise. The path
    // may be a "pak://" entry of the mounted assets.pak (AssetPak), so read
    // it through open()/openPath() rather than a raw ifstream.
    ...
    // Opens a path resolve()/PlatformResources::resolveExisting() returned,
    // loose file or pak entry alike. Returns nullptr if it cannot be opened.
