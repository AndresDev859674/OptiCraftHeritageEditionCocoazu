...
static std::vector<byte_t> gzipDecompress(const std::vector<unsigned char> &compressed)
{
...
#ifdef PS2_PLATFORM
                if (!PlatformStorage::readFile(cf.getFile()->toString(), compressed))
                    throw std::runtime_error("chunk read failed");
#else
                std::ifstream fin(cf.getFile()->toString(), std::ios::binary);
                compressed.assign(std::istreambuf_iterator<char>(fin), std::istreambuf_iterator<char>());
#endif
