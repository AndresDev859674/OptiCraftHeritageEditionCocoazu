...
public:
	...
	// Resolved once by TileEntityRenderer::getSpecialRendererForEntity: the
	// lookup keys a std::map by typeid name and falls back to a dynamic_cast
	// chain, and it ran for every visible tile entity every frame. The type of
	// a tile entity never changes, so the answer is stable for its lifetime.
	TileEntitySpecialRenderer *cachedSpecialRenderer;
	bool specialRendererResolved;
