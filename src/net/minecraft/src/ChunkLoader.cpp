...
namespace
{
...
#if PLATFORM_PS2
	int_t readPackedNibble(const std::vector<byte_t> &bytes, std::size_t index)
	{
		const byte_t packed = bytes[index >> 1];
		return (index & 1u) == 0u ? (packed & 0xf) : ((packed >> 4) & 0xf);
	}

	void writePackedNibble(std::vector<byte_t> &bytes, std::size_t index, int_t value)
	{
		byte_t &packed = bytes[index >> 1];
		if ((index & 1u) == 0u)
			packed = (byte_t)((packed & 0xf0) | (value & 0xf));
		else
			packed = (byte_t)((packed & 0x0f) | ((value & 0xf) << 4));
	...
	Chunk *loadLegacyChunkFast(World *world, int_t chunkX, int_t chunkZ,
	                           const std::vector<byte_t> &blocks,
	                           const std::vector<byte_t> *metadata,
	                           ...
	                           const std::vector<byte_t> *blockLight)
	{
...
#endif
		std::unique_ptr<Chunk> chunk(new Chunk(world, chunkX, chunkZ));
		ExtendedBlockStorage **storage = chunk->getBlockStorageArray();
		std::vector<byte_t> *sectionBlocks[Chunk::SECTION_COUNT] = {};
		int_t blockCounts[Chunk::SECTION_COUNT] = {};
		int_t randomTickCounts[Chunk::SECTION_COUNT] = {};
		...
		for (int_t x = 0; x < 16; ++x)
		{
			for (int_t z = 0; z < 16; ++z)
			{
				for (int_t y = 0; y < kLegacyHeight; ++y)
				{
					const std::size_t sourceIndex = (std::size_t)(x << 11 | z << 7 | y);
					const byte_t sourceBlock = blocks[sourceIndex];
					if ((sourceBlock & 0xff) == 0)
						continue;

					const int_t sectionIndex = y >> 4;
					if (storage[sectionIndex] == nullptr)
					{
						storage[sectionIndex] = new ExtendedBlockStorage(sectionIndex << 4);
						sectionBlocks[sectionIndex] = &storage[sectionIndex]->func_48692_g();
					}

					const byte_t mappedBlock = ChunkBlockMap::remapBlock(sourceBlock);
					const int_t blockId = mappedBlock & 0xff;
					if (blockId == 0)
						continue;

					const std::size_t storageIndex = (std::size_t)(((y & 15) << 8) | (z << 4) | x);
					(*sectionBlocks[sectionIndex])[storageIndex] = mappedBlock;
					++blockCounts[sectionIndex];
					if (blockId < Block::BLOCK_REGISTRY_SIZE && Block::blocksList[blockId] != nullptr &&
					    Block::tickOnLoad[blockId])
					{
						++randomTickCounts[sectionIndex];
					}
				}
			}
		}
		...
		for (int_t sectionIndex = 0; sectionIndex < Chunk::SECTION_COUNT; ++sectionIndex)
		{
			...
			section->setBulkBlockCounts(blockCounts[sectionIndex], randomTickCounts[sectionIndex]);
			std::vector<byte_t> *metadataData = metadata != nullptr ? &section->func_48697_j().data : nullptr;
			std::vector<byte_t> *skyLightData = skyLight != nullptr ? &section->getSkylightArray().data : nullptr;
			std::vector<byte_t> *blockLightData = blockLight != nullptr ? &section->getBlocklightArray().data : nullptr;

			if (metadataData == nullptr && skyLightData == nullptr && blockLightData == nullptr)
				continue;
			...
			for (int_t x = 0; x < 16; ++x)
			{
				for (int_t z = 0; z < 16; ++z)
				{
					...
					{
						const int_t y = sectionBaseY + localY;
						const std::size_t sourceIndex = (std::size_t)(x << 11 | z << 7 | y);
						const std::size_t storageIndex = (std::size_t)((localY << 8) | (z << 4) | x);
						if (metadataData != nullptr)
							writePackedNibble(*metadataData, storageIndex, readPackedNibble(*metadata, sourceIndex));
						if (skyLightData != nullptr)
							writePackedNibble(*skyLightData, storageIndex, readPackedNibble(*skyLight, sourceIndex));
						if (blockLightData != nullptr)
							writePackedNibble(*blockLightData, storageIndex, readPackedNibble(*blockLight, sourceIndex));
					}
				}
			}
		}
...
{
	Chunk *chunk = loadChunkBlocksFromCompound(world, nbttagcompound);
	if (chunk != nullptr)
		loadChunkEntitiesFromCompound(world, chunk, nbttagcompound);
	return chunk;
}

Chunk *ChunkLoader::loadChunkBlocksFromCompound(World *world, NBTTagCompound *nbttagcompound)
{
...
#if PLATFORM_PS2
	const std::vector<byte_t> &blocks = nbttagcompound->getByteArrayRef("Blocks");
#else
...
#if PLATFORM_PS2
	const std::vector<byte_t> &dataBytes = nbttagcompound->getByteArrayRef("Data");
	const std::vector<byte_t> &skyBytes = nbttagcompound->getByteArrayRef("SkyLight");
	const std::vector<byte_t> &blockLightBytes = nbttagcompound->getByteArrayRef("BlockLight");
	const bool validData = dataBytes.size() == NIBBLE_SIZE;
	const bool validSkyLight = skyBytes.size() == NIBBLE_SIZE;
	const bool validBlockLight = blockLightBytes.size() == NIBBLE_SIZE;
	...
	Chunk *chunk = loadLegacyChunkFast(world, i, j, blocks,
	                                  validData ? &dataBytes : nullptr,
	                                  validSkyLight ? &skyBytes : nullptr,
	                                  validBlockLight ? &blockLightBytes : nullptr);
	if (!validBlockLight)
...
#if PLATFORM_PS2
	const std::vector<byte_t> &hm = nbttagcompound->getByteArrayRef("HeightMap");
#else
	...
	{
#if PLATFORM_PS2 && MC_LOG_LEVEL > 2
		const std::uint32_t lightingRegenStart = platformProfileRenderPhaseBegin();
#endif
		chunk->generateSkylightMap();
#if PLATFORM_PS2 && MC_LOG_LEVEL > 2
		platformProfileChunkDecode(lightingRegenStart, PlatformChunkDecodeStage::Lighting);
#endif
	}
	return chunk;
}

void ChunkLoader::loadChunkEntitiesFromCompound(World *world, Chunk *chunk, NBTTagCompound *nbttagcompound)
{
#if PLATFORM_PS2 && MC_LOG_LEVEL > 2
