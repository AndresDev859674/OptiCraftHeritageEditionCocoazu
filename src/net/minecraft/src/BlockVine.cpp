...
bool BlockVine::canPlaceBlockOnSide(World *world, int_t x, int_t y, int_t z, int_t side)
{
    switch (side)
    {
        case 1: return canBePlacedOn(world->getBlockIdForPopulation(x, y + 1, z));
        case 2: return canBePlacedOn(world->getBlockIdForPopulation(x, y, z + 1));
        case 3: return canBePlacedOn(world->getBlockIdForPopulation(x, y, z - 1));
        case 4: return canBePlacedOn(world->getBlockIdForPopulation(x + 1, y, z));
        case 5: return canBePlacedOn(world->getBlockIdForPopulation(x - 1, y, z));
...
{
    return blockId > 0 && blockId < BLOCK_REGISTRY_SIZE && Block::vineAttachableLookup[blockId];
}
...
bool BlockVine::canVineStay(World *world, int_t x, int_t y, int_t z)
{
    ...
    if (validMetadata == 0 && !canBePlacedOn(world->getBlockIdForPopulation(x, y + 1, z)))
        return false;
