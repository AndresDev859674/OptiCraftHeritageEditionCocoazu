...
MapDataBase *createMapData(const std::string &name)
{
...
{
	return loadData(s, classType == typeid(MapData) ? &createMapData : nullptr);
}

MapDataBase* MapStorage::loadData(const std::string &s, MapDataFactory factory)
{
...
{
	if (saveHandler != nullptr && saveHandler->isReadOnly())
		return;
...
{
	if (saveHandler == nullptr || saveHandler->isReadOnly()) return;
	try
	{
