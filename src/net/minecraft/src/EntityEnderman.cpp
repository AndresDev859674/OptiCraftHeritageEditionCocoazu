...
bool EntityEnderman::getCanSpawnHere()
{
#if PLATFORM_END_MAX_ENDERMEN > 0
	// The End biome spawns nothing but endermen, in groups of four, so the
	// global live-mob cap alone lets one spawn pass fill every slot with them.
	// Counted here, at spawn time, so the group loop stops at the cap too.
	if (worldObj->worldProvider != nullptr && worldObj->worldProvider->worldType == 1)
	{
		int_t endermen = 0;
		for (Entity *entity : worldObj->loadedEntityList)
		{
			if (entity != this && !entity->isDead && dynamic_cast<EntityEnderman *>(entity) != nullptr &&
				++endermen >= PLATFORM_END_MAX_ENDERMEN)
				return false;
		}
	}
#endif
	return EntityMob::getCanSpawnHere();
}
...
bool EntityEnderman::teleportTo(double x, double y, double z)
{
	...
	constexpr int_t particleCount = PLATFORM_ENDERMAN_TELEPORT_PARTICLES;
