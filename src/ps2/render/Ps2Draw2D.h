...
// Submit whatever the 2D path still has staged.
//
// With PS2_BATCH_2D_SPRITES the batch survives the call that filled it, so the
// GUI's one-quad-per-draw traffic can share a packet. Anything that programs GS
// state, draws through another path, or ends the frame must call this FIRST:
// staged sprites have to reach the GS while the state they were built under is
// still the state the GS holds. Cheap and idempotent when nothing is staged.
