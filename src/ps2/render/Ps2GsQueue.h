...
struct Ps2GsQueueRenderStats
{
	...
	// EE cycles spent inside the guard's synchronous flush. A flush is a
	// gsKit_queue_exec plus a dmaKit_wait_fast, i.e. the EE stalls until the GS
	// has drained; `flushes` alone cannot say whether that stall is a rounding
	// error or the bulk of a draw call.
	unsigned long flushCycles = 0;
};
