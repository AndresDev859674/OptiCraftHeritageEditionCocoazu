...
namespace
{
    ...
    // Staging that outlives one ps2_draw_2d_mesh call.
    //
    // Gui::drawTexturedModalRect opens the tessellator, emits four vertices and
    // closes it again, so every heart, hunger shank and hotbar slot arrives as
    // its own draw. Measured in the tutorial world the HUD was 213 sprites and
    // 11.2ms per frame -- about 52us to put one axis-aligned quad on screen,
    // which is packet and state overhead, not rasterisation.
    //
    // The batch therefore persists across calls and is flushed by a barrier
    // rather than by the end of the call. What it must never do is outlive a
    // change to the state it was built under, so every GS register write in
    // Ps2RenderGsState calls ps2_draw_2d_flush_pending() *before* writing:
    // sprites staged under the old state have to reach the GS while that state
    // is still programmed.
    struct PendingBatch
    {
        GSGLOBAL* gsGlobal = nullptr;
        GSTEXTURE* texture = nullptr;
        void (*queueGuard)(int vertexCount) = nullptr;
        int triangles = 0;
        int sprites = 0;
        bool clampValid = false;
        Ps2ClampSel clamp = { -1, -1, -1 };
    };

    PendingBatch s_pending;
    // ps2_draw_2d_flush_pending is called from the GS state module, which the
    // flush itself reaches through gsKit. Without this the first flush would
    // re-enter and empty a batch it is already emitting.
    bool s_flushing = false;

    GSPRIMSTQPOINT s_batch[PS2_DRAW2D_BATCH_TRIS * 3] __attribute__((aligned(16)));
    GSPRIMUVPOINT s_spriteBatch[PS2_DRAW2D_BATCH_SPRITES * 2] __attribute__((aligned(16)));

    void flushPendingTriangles()
    {
        if (s_pending.triangles <= 0 || s_pending.gsGlobal == nullptr)
            return;
        const int triangles = s_pending.triangles;
        s_pending.triangles = 0;
        if (s_pending.queueGuard)
            s_pending.queueGuard(triangles * 3);
        gsKit_prim_list_triangle_goraud_texture_stq_3d(
            s_pending.gsGlobal, s_pending.texture, triangles * 3, s_batch);
    }

    void flushPendingSprites()
    {
        if (s_pending.sprites <= 0 || s_pending.gsGlobal == nullptr)
            return;
        const int sprites = s_pending.sprites;
        s_pending.sprites = 0;
        if (s_pending.queueGuard)
            s_pending.queueGuard(sprites * 2);
        gskit_prim_list_sprite_texture_uv_3d(
            s_pending.gsGlobal, s_pending.texture, sprites * 2, s_spriteBatch);
    }
...
void ps2_draw_2d_flush_pending()
{
    if (s_flushing)
        return;

    s_flushing = true;
    // Triangles first: a batch never holds both kinds, because each staging
    // point flushes the other, so this preserves submission order rather than
    // choosing one.
    flushPendingTriangles();
    flushPendingSprites();
    s_flushing = false;

    // Drop the cached clamp selection with the batch.
    //
    // The selection is CPU-side state that mirrors what the GS was last told,
    // and the 3D path writes CLAMP through the same register. Every reason to
    // reach this function is something else about to drive the GS, so the mirror
    // cannot be trusted afterwards; keeping it would let the next sprite sample
    // under a clamp mode the GS no longer holds. Costs one redundant clamp write
    // per batch and removes the whole class of bug.
    s_pending.clampValid = false;
}
...
bool ps2_draw_2d_mesh(const Ps2NativeMeshView& mesh, const Ps2Draw2DState& state)
{
    ...
    // Staging carries over between calls, so a batch already holding sprites
    // built against another texture or GS context has to leave first.
    if (s_pending.triangles > 0 || s_pending.sprites > 0) {
        if (s_pending.texture != state.texture || s_pending.gsGlobal != state.gsGlobal)
            ps2_draw_2d_flush_pending();
    }
    s_pending.gsGlobal = state.gsGlobal;
    s_pending.texture = state.texture;
    s_pending.queueGuard = state.queueGuard;

    int& batchTriangles = s_pending.triangles;
    int& batchSprites = s_pending.sprites;
    bool& clampValid = s_pending.clampValid;
    Ps2ClampSel& lastClamp = s_pending.clamp;

    auto flushBatch = []() { flushPendingTriangles(); };
    auto flushSprites = []() { flushPendingSprites(); };
    ...
    auto flushPending = []() { ps2_draw_2d_flush_pending(); };
    ...
    auto emitTriangle = [&](int i0, int i1, int i2) {
        ...
        GSPRIMSTQPOINT* point = &s_batch[batchTriangles * 3];
    ...
    auto emitQuadSprite = [&](int i0, int i1, int i2, int i3) -> bool {
        ...
        GSPRIMUVPOINT* point = &s_spriteBatch[batchSprites * 2];
