...
struct Ps2GreedyRawTarget
{
	std::vector<int_t> *raw;
	int_t originX;
	int_t originY;
	int_t originZ;
};
...
// Snapshot one 16x16x16 section and derive all six visible-face row masks.
// Interior visibility is resolved from 16-bit row masks; only section borders
// query ChunkCache, reducing repeated neighbour probes across greedy passes.
...
// Same scan/merge logic, but append the canonical six-slot PS2 terrain capture
// layout directly instead of round-tripping through Tessellator::capture().
