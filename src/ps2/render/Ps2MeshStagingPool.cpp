...
int ps2_mesh_staging_acquire()
{
	for (int slot = 0; slot < PS2_MESH_STAGING_SLOTS; ++slot)
	{
		...
		s_slots[slot].sectionCache.valid = false;
...
Ps2MeshSectionCache *ps2_mesh_staging_section_cache(int slot)
{
	if (slot < 0 || slot >= PS2_MESH_STAGING_SLOTS || !s_slots[slot].leased)
		return nullptr;
	return &s_slots[slot].sectionCache;
}

std::size_t ps2_mesh_staging_ram_bytes()
{
	std::size_t bytes = sizeof(Ps2MeshSectionCache) * PS2_MESH_STAGING_SLOTS;
	for (int slot = 0; slot < PS2_MESH_STAGING_SLOTS; ++slot)
	{
...
{
	return s_leasedCount < PS2_MESH_STAGING_SLOTS;
}
