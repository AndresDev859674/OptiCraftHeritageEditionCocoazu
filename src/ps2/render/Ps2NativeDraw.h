...
struct Ps2NativeFrameContext {
    // 16-byte aligned so the COP2 kernels in Ps2Vu0Math.h can lqc2/sqc2 the
    // columns directly. Both matrices are copied around by value inside larger
    // structs, so the guarantee has to live on the member itself rather than on
    // any one instance.
