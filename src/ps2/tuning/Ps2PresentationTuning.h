...
// 8: every animated tile is already off in the PS2 profile (GameSettings
// ofAnimated*), so what runs here is the compass and the clock on items.png
// plus the level-1/2 mip re-sends they now cause; 2.5 updates a second is
// plenty for a needle.
#define PS2_DYNAMIC_TEXTURE_INTERVAL_TICKS 8
...
// Legacy UI keeps its own fixed scale so console layouts are independent of
// the classic GUI scale policy. Create New World uses a narrower panel below.
#define PS2_LEGACY_GUI_SCALE 2.0
#define PS2_LEGACY_CREATE_WORLD_PANEL_WIDTH 208
#define PS2_FORCE_GUI_SCALE 2

// Depth mapping. See the long note on ps2_gs_depth in Ps2ClipGuard.h: the GS
// depth buffer is PSMZ_16 (16 usable bits) and the textbook z_ndc mapping is
...
//   1 = Z proportional to eye distance (W buffer), ~1024 units/block everywhere
